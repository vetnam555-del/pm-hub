...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const base=`http://127.0.0.1:${server.address().port}`;let browser;try{
 ...
 await page.locator('[data-view="preview"]').click();assert.ok((await page.locator('#mix-body').innerText()).includes('필수 입력 및 예산 검수를 통과'));
 ...
 await page.locator('.mix-proposal > summary').click();await page.locator('[data-field="client"]').fill('변경 광고주');assert.equal(await page.locator('[data-field="approved"]').isChecked(),false,'client change invalidates approval');
 await page.locator('.mix-storage > summary').click();await page.locator('[data-action="save-plan"]').click({clickCount:1});assert.equal(await page.locator('#mix-saved option').count(),2,'first click after typing saves plan');
 ...
 await page.locator('summary').filter({hasText:'단가 시나리오'}).click();await page.locator('[data-field="scenario"]').selectOption('120');assert.equal(await page.locator('[data-field="scenario"]').inputValue(),'120');
 await page.locator('.mix-storage > summary').click();await page.locator('[data-action="save-plan"]').click();await page.reload();assert.equal(await page.locator('#mix-saved option').count(),3);
 await page.setViewportSize({width:390,height:844});await page.emulateMedia({reducedMotion:'reduce'});await page.goto(base+'/#tool-mediamix');await page.locator('.mix-studio .tool-hero').waitFor();await page.screenshot({path:path.join(out,'compose-mobile.png'),fullPage:true});assert.ok(await page.evaluate(()=>document.querySelector('.mix-studio').getBoundingClientRect().width<=window.innerWidth));
 ...
 await page.goto(base+'/#benchmark');await page.locator('#mix-open-library').click();await page.locator('#mix-industry').waitFor();
 ...
 const smallPack={version:1,benchmarks:[{id:'qa-default',industry:'QA',brand:'QA',product:'meta-leads',source:'QA',model:'',goal:'',sourceKind:'',markup:''}]};
 ...
 await page.locator('#mix-industry').selectOption('QA');await page.locator('#mix-media-filter').selectOption('');await page.locator('[data-bench]').first().click();assert.equal(await page.locator('[data-field="model"]').inputValue(),'CPC');assert.equal(await page.locator('[data-field="goal"]').inputValue(),'리드');assert.equal(await page.locator('[data-scope="row"][data-field="markup"]').inputValue(),'0');
