...
(function () {
  ...
  let db={plan:E.plan(),saved:[],benchmarks:[]}, selected=-1, page='compose', filter='', brand='', mediaFilter='', notice='';
  try {const s=JSON.parse(localStorage.getItem(key));if(s?.plan&&Array.isArray(s.plan.rows)&&Array.isArray(s.saved)&&Array.isArray(s.benchmarks))db=s;}catch(_){}
  const p=()=>db.plan, root=()=>document.getElementById('page-tool-mediamix');
  function save(){try{localStorage.setItem(key,JSON.stringify(db));notice='이 브라우저에 저장됨';}catch(_){notice='저장 실패: JSON 백업을 내려받으세요.';}}
  ...
  function compose(res) {
    ...
    return `<section class="mix-section"><h2>제안 정보</h2><div class="mix-fields">${field('client','광고주')}${field('title','제안명')}${field('agency','작성 주체')}${field('date','작성일','date')}${field('start','시작일','date')}${field('end','종료일','date')}</div></section>
      ...
      <div class="mix-scroll"><table><thead><tr><th>매체 / 캠페인</th><th>기기</th><th>기준</th><th>공급가 예산</th><th>실매체비</th><th>예상 클릭</th><th>예상 전환</th><th>검토</th><th></th></tr></thead><tbody>${res.rows.map((x,i)=>`<tr class="${i===selected?'selected':''}"><td><button data-edit="${i}">${esc(x.name)}<br><strong>${esc(x.type)}</strong></button></td><td>${esc(x.device)}</td><td>${esc(x.model)}</td><td>${n(x.amount)}</td><td>${n(x.media)}</td><td>${n(x.clicks)}</td><td>${n(x.conv,1)} <small>${esc(x.goal)}</small></td><td>${x.approved?'확인':'미확인'}</td><td><button data-remove="${i}" title="캠페인 삭제" aria-label="${i+1}행 삭제">×</button></td></tr>`).join('')||'<tr><td colspan="9">선택된 캠페인 없음</td></tr>'}</tbody><tfoot><tr><th colspan="3">합계</th><td>${n(res.totals.amount)}</td><td>${n(res.totals.media)}</td><td>${n(res.totals.clicks)}</td><td>${n(res.totals.conv,1)}</td><td colspan="2"></td></tr></tfoot></table></div></section>
  ...
  function preview(res) {return checks(res)+`<div class="mix-actions mix-section">${button('xlsx','제출용 XLSX','mix-primary')}${button('draft','검토용 XLSX')}${button('print','인쇄 / PDF')}</div><article class="mix-paper"><h2>${esc(p().client||'광고주 미입력')} · ${esc(p().title)}</h2><p>${esc(p().start)} ~ ${esc(p().end)} / ${esc(p().agency)} / ${esc(p().date)}</p><h3>예상 미디어플랜</h3><div class="mix-scroll"><table><thead><tr><th>매체 / 캠페인</th><th>공급가</th><th>예상 노출</th><th>예상 클릭</th><th>CTR</th><th>예상 전환</th><th>예상 매출</th><th>ROAS</th></tr></thead><tbody>${res.rows.map(r=>`<tr><td>${esc(r.name)}<br>${esc(r.type)}</td><td>${n(r.amount)}</td><td>${n(r.impr)}</td><td>${n(r.clicks)}</td><td>${n(r.ctr,2)}%</td><td>${n(r.conv,1)} ${esc(r.goal)}</td><td>${n(r.revenue)}</td><td>${n(r.roas,1)}%</td></tr>`).join('')}</tbody></table></div><p>공급가 ${n(res.totals.amount)}원 + VAT ${n(res.totals.tax)}원 = 청구 총액 ${n(res.totals.gross)}원</p><p>예상치는 성과를 보장하지 않습니다. 단가·CTR·CVR은 계획 가정이며 예산, 기간, 소재, 타기팅에 따라 달라집니다. CPC·CPM은 실매체비 기준, CPA·ROAS는 수수료 포함 공급가 기준입니다. 미산출 항목은 —로 표시하며 구매·리드·설치는 합산하지 않습니다. 메시지 발송량은 클릭수에 합산하지 않습니다.</p>${res.errors.length?'<strong>검토용 초안 · 미완료 항목 존재</strong>':''}${res.warnings.length?`<ul>${res.warnings.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:''}</article>`;}
  function addBench(id) {
    const b=allBench().find(x=>x.id===id);if(!b)return;
    p().rows.push({...E.row(b.product),...copy(b),approved:false,locked:b.model==='FIXED',amount:b.amount||0,weight:b.amount||1,source:b.source,rate:b.rate??'',ctr:b.ctr??'',cvr:b.cvr??'',aov:b.aov??''});selected=p().rows.length-1;page='compose';save();render();
  }
  ...
  async function importFile(file){
    if(!file)return;try{
      ...
      pending=file.name.endsWith('.xlsx')?await window.MixWorkbook.importBench(await file.arrayBuffer(),file.name):JSON.parse(await file.text());
      if(pending.kind==='mix-backup'){
        if(!pending.db?.plan||!Array.isArray(pending.db.plan.rows)||pending.db.plan.rows.length>300||!Array.isArray(pending.db.saved))throw Error('백업 구조 오류');
        E.validatePack({version:1,benchmarks:pending.db.benchmarks});E.compute(pending.db.plan);
      }else E.validatePack(pending);
      const count=pending.kind==='mix-backup'?pending.db.benchmarks.length:pending.benchmarks.length;
      document.getElementById('mix-import-summary').textContent=pending.kind==='mix-backup'?`백업 복원: 현재 계획과 자료 ${count}건으로 교체합니다. 기존 내용은 복원 전 JSON으로 자동 다운로드합니다.`:`${count}건을 추가합니다. 동일 ID는 건너뛰며 원본 파일은 수정하지 않습니다.`;
  ...
  async function act(action) {
    try {
      ...
      if(action==='commit-import'){
        if(pending.kind==='mix-backup'){download(JSON.stringify({kind:'mix-backup',db},null,2),'미디어믹스_복원전백업.json','application/json');db=pending.db;}
      ...
      if(action==='backup'){download(JSON.stringify({kind:'mix-backup',db},null,2),'미디어믹스_백업.json','application/json');return;}
      ...
      if(action==='duplicate'){db.saved.push(copy(p()));db.plan=copy(p());p().title+=' (복제)';}
  ...
  function bind(){
    ...
    root().querySelectorAll('[data-field]').forEach(el=>el.onchange=()=>{const obj=el.dataset.scope==='row'?p().rows[selected]:p();obj[el.dataset.field]=el.type==='checkbox'?el.checked:el.value;if(el.dataset.scope==='row'&&el.dataset.field!=='approved')obj.approved=false;if(el.dataset.field==='scenario')p().rows.forEach(r=>r.approved=false);save();render();});
